...
possible_ms = [number * 0.1 for number in range(-100,101)]
...
possible_bs = [number * 0.1 for number in range (-200,201)]
...
for ms in possible_ms:
    for bs in possible_bs:
        xyz = calculate_all_error(ms, bs, datapoints)
        if xyz < smallest_error:
            smallest_error = xyz
            best_m = ms
            best_b = bs
        else:
...
print(round(best_m, 1), round(best_b, 1), round(smallest_error, 1))
